  ...
  return (
    <table className="w-full block md:table mt-[8px] md:border-separate md:border-spacing-0 md:rounded-[12px] md:shadow-[0_1px_10px_#ffd99a10] md:dark:shadow-none md:bg-[#fffdfa] md:dark:bg-slate-900 transition-colors duration-300">
      ...
      <tbody className="block md:table-row-group">
        ...
        {tasks.map((task, idx) => (
          <tr key={task.id} className={`block md:table-row bg-white dark:bg-slate-800 md:bg-transparent rounded-xl md:rounded-none mb-4 md:mb-0 p-4 md:p-0 shadow-sm md:shadow-none border border-orange-100 dark:border-slate-700 md:border-none group hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${isCompleted ? 'opacity-80 md:opacity-100' : ''}`}>
            ...
            <td className={`${tdStyles} flex flex-col items-start md:items-center ${isCompleted ? 'text-[#888] dark:text-slate-500' : ''}`}>
              <div className="flex flex-wrap items-center gap-2 mb-2 md:mb-0">
                <span className={!isCompleted && task.last_date === todayDate ? 'text-black dark:text-white font-bold text-lg md:text-base' : `text-black dark:text-slate-200 text-lg md:text-base ${isCompleted ? '' : 'font-semibold md:font-normal'}`}>
              ...
              {task.tags && task.tags.length > 0 && (
                <div className={`flex flex-wrap gap-1 mt-1 justify-start md:justify-center ${isCompleted ? 'opacity-70' : ''}`}>
                  {task.tags.map(tag => (
