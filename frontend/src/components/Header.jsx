...
// ✨ ADDED isOnline TO THE PROPS LIST
function Header({ isDarkMode, setIsDarkMode, isSyncing, hasUnsavedChanges, handleSyncRecovery, isOnline }) {
  return (
      ...
      <div className="flex flex-1 items-center gap-3">
        ...
        <div className="hidden sm:flex items-center gap-2 text-xs font-semibold px-2.5 py-1 rounded-full bg-white/50 dark:bg-slate-800/50 border border-orange-100 dark:border-slate-700 text-slate-600 dark:text-slate-300 shadow-sm">
          {isSyncing ? (
            ...
            <><span className="text-amber-500 text-[10px]">⬤</span> Offline Queue</>
          ) : (
            <><span className="text-green-500 text-[10px] animate-pulse">⬤</span> Live</>
          )}
        </div>

        {/* ✨ UPDATED: ONLY SHOW RECOVERY BUTTON IF ONLINE AND STUCK */}
        {(hasUnsavedChanges && isOnline) && (
          <button 
            onClick={handleSyncRecovery}
            className="ml-3 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 text-xs font-bold px-3 py-1.5 rounded-full hover:bg-red-200 dark:hover:bg-red-800/50 transition-colors flex items-center gap-1 shadow-sm animate-pulse"
            title="Click to clear corrupted local data and restore from server"
          >
            <span>⚠️</span> Fix Sync Error
          </button>
        )}
      </div>
