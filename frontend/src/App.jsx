...
//   const correctPassword = process.env.REACT_APP_TASKS_PASSWORD;
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      setTasks(tasks || []);
      setPasswordOk(true); // Only unlocks if the server returns 200 OK
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { tasks: currentTasks } = await apiStorage.getTasks(enteredPassword);
      ...
      await apiStorage.saveTasks(updatedTasks, enteredPassword);
  ...
  // Replaced CSS class names with Tailwind background colors
  ...
  // Shared classes for form inputs
  ...
  // Shared classes for table elements
          ...
          <div className="bg-[#fff8e1] py-[2rem] px-[3rem] rounded-[12px] shadow-[0_8px_16px_rgba(0,0,0,0.2)] w-[320px] text-center">
            ...
            <form onSubmit={handlePasswordSubmit}>
              <input
                ...
                autoFocus
              />
              <button type="submit" className="bg-[#f57c00] text-white border-none p-[0.75rem] w-full rounded-[8px] font-bold cursor-pointer transition-colors hover:bg-[#ef6c00]">
                Unlock
              </button>
            </form>
          </div>
        ...
        <div>
          <div className="min-h-screen flex flex-col items-center w-screen">
            <div className="w-[92vw] max-w-[512px] rounded-[21px] mb-[29px] shadow-[0_7px_36px_#ff944740] px-[28px] pt-[34px] pb-[24px] backdrop-blur-[2.5px] bg-[linear-gradient(107deg,#ffd59e_58%,#ffe7cc_100%)]">
              ...
              {editingTaskId && (
                <p className="text-[#f57c00] font-semibold p-[8px] text-center -mt-[10px] mb-[10px]">
              ...
              <form onSubmit={handleSubmit} className="flex flex-wrap gap-y-[14px] gap-x-[22px] items-center justify-center flex-row">
                ...
                <div className="w-full flex justify-center mt-[13px]">
                  <button 
                    type="submit" 
                    disabled={isLoading} 
                    className={`bg-[linear-gradient(90deg,#ff9100_50%,#ffb451_100%)] text-white border-none rounded-[9px] px-[30px] py-[9px] text-[1.1em] font-bold shadow-[0_1px_6px_#ffd08555] cursor-pointer transition-colors hover:bg-[#f27300] flex gap-[8px] items-center justify-center ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
                  >
                    {isLoading && <span className="animate-spin inline-block w-[14px] h-[14px] border-[2px] border-[rgba(255,255,255,0.3)] border-t-white rounded-full align-middle mr-[8px]"></span>}
                    {isLoading 
                  ...
                  {editingTaskId && (
                    <button
                      type="button"
                      className="bg-[#f3f3f3] border-none rounded-[8px] px-[16px] py-[8px] ml-[12px] text-[1em] font-medium text-[#333] cursor-pointer transition-colors hover:bg-[#e0e0e0]"
                      onClick={() => {
                        setTaskName('');
                        setFactor('Easy');
                        setLastDate('');
            ...
            <div className="w-[99vw] max-w-[900px] mx-auto mt-[34px] rounded-[18px] shadow-[0_2px_14px_#ffe5a940] bg-[#fffbe7] pt-[28px] px-[18px] pb-[22px]">
              ...
              <table className="w-full border-separate border-spacing-0 mt-[8px] rounded-[12px] shadow-[0_1px_10px_#ffd99a10] bg-[#fffdfa]">
                ...
                <tbody>
                  {sortedTasks.length === 0 &&(
                    <tr>
                      <td colSpan="5" className="py-[10px] px-[8px] text-center">No tasks added.</td>
                    </tr>     
                  )}
                  {sortedTasks.map((task, idx) => (
                    <tr key={task.id} className="group">
                      ...
                      <td className={tdStyles}>
                        <span className={`inline-block min-w-[71px] text-[.96em] font-bold text-white rounded-[16px] py-[3px] px-[17px] mr-[7px] tracking-[1px] align-middle ${getFactorClass(task.factor)}`}>
                          {task.factor}
                        </span>
                      </td>
                      ...
                      <td className={tdStyles}>
                        <div className="flex gap-[10px] justify-center">
                          <button
                            className="bg-transparent border-none text-[#065fd4] text-[1.15em] cursor-pointer py-[3px] px-[6px] transition-colors rounded-[6px] hover:text-[#004bb8] hover:bg-[#e8f0fe]"
                          ...
                          <button
                            className="bg-transparent border-none text-[#e34d4d] text-[1.2em] cursor-pointer py-[3px] px-[6px] transition-colors rounded-[6px] hover:text-[#be2323] hover:bg-[#fff0f0]"
                          ...
                          <button
                            className="bg-transparent border-none text-[#2e7d32] text-[1.25em] cursor-pointer py-[3px] px-[5px] transition-colors rounded-[6px] hover:text-[#0d540d] hover:bg-[#e8f5e9]"
                    ...
                    {[...tasks]
                      ...
                      .map((task, idx) => (
                        <tr key={task.id} className="group">
                          ...
                          <td className={tdStyles}>
                            <button
                              className="bg-transparent border-none text-[1.15em] cursor-pointer py-[2px] px-[5px] text-[#f89c0e] hover:text-[#d37800]"
